...
public class TcpPacketEncryptor extends ByteToMessageCodec<ByteBuf> {
   ...
   public void encode(ChannelHandlerContext ctx, ByteBuf in, ByteBuf out) throws Exception {
      if (this.session.getPacketProtocol().getEncryption() != null) {
         ...
         out.writeBytes((byte[])this.encryptedArray, 0, this.session.getPacketProtocol().getEncryption().encrypt(bytes, 0, length, this.encryptedArray, 0));
      } else {
         out.writeBytes(in);
      }
   ...
   private byte[] getBytes(ByteBuf buf) {
      ...
      buf.readBytes((byte[])this.decryptedArray, 0, length);
