   ...
   public void packetReceived(PacketReceivedEvent event) {
      ...
      if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
         if (event.getPacket() instanceof LoginStartPacket) {
            ...
            boolean verify = event.getSession().hasFlag("verify-users") ? (Boolean)event.getSession().getFlag("verify-users") : true;
            if (verify) {
               event.getSession().send(new EncryptionRequestPacket(this.serverId, KEY_PAIR.getPublic(), this.verifyToken));
            } else {
               (new Thread(new ServerListener.UserAuthTask(event.getSession(), (SecretKey)null))).start();
            }
            ...
            (new Thread(new ServerListener.UserAuthTask(event.getSession(), key))).start();
         }
      }
      ...
      public void run() {
         ...
         if (verify && this.key != null) {
            ...
            try {
               ...
               this.session.disconnect("Failed to make session service request.", var5);
               return;
            }
         ...
         int threshold = (Integer)this.session.getFlag("compression-threshold");
