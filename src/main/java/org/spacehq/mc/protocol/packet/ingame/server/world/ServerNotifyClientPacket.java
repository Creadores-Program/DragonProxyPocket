...
public class ServerNotifyClientPacket implements Packet {
   ...
   public void read(NetInput in) throws IOException {
      this.notification = (ClientNotification)MagicValues.key(ClientNotification.class, in.readUnsignedByte());
      float value = in.readFloat();
      if (this.notification == ClientNotification.CHANGE_GAMEMODE) {
         this.value = (ClientNotificationValue)MagicValues.key(GameMode.class, (int)value);
      } else if (this.notification == ClientNotification.DEMO_MESSAGE) {
         this.value = (ClientNotificationValue)MagicValues.key(DemoMessageValue.class, (int)value);
   ...
   public void write(NetOutput out) throws IOException {
      out.writeByte((Integer)MagicValues.value(Integer.class, this.notification));
      float value = 0.0F;
      if (this.value instanceof GameMode) {
         value = (float)(Integer)MagicValues.value(Integer.class, (Enum)this.value);
      }
