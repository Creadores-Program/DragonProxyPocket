...
public class ClientWindowActionPacket implements Packet {
   ...
   public void read(NetInput in) throws IOException {
      ...
      this.action = (WindowAction)MagicValues.key(WindowAction.class, in.readByte());
      this.clicked = NetUtil.readItem(in);
      if (this.action == WindowAction.CLICK_ITEM) {
         this.param = (WindowActionParam)MagicValues.key(ClickItemParam.class, param);
      } else if (this.action == WindowAction.SHIFT_CLICK_ITEM) {
         this.param = (WindowActionParam)MagicValues.key(ShiftClickItemParam.class, param);
      } else if (this.action == WindowAction.MOVE_TO_HOTBAR_SLOT) {
         this.param = (WindowActionParam)MagicValues.key(MoveToHotbarParam.class, param);
      } else if (this.action == WindowAction.CREATIVE_GRAB_MAX_STACK) {
         this.param = (WindowActionParam)MagicValues.key(CreativeGrabParam.class, param);
      } else if (this.action == WindowAction.DROP_ITEM) {
         this.param = (WindowActionParam)MagicValues.key(DropItemParam.class, param + (this.slot != -999 ? 2 : 0));
      } else if (this.action == WindowAction.SPREAD_ITEM) {
         this.param = (WindowActionParam)MagicValues.key(SpreadItemParam.class, param);
      } else if (this.action == WindowAction.FILL_STACK) {
         this.param = (WindowActionParam)MagicValues.key(FillStackParam.class, param);
      }
   ...
   public void write(NetOutput out) throws IOException {
      ...
      int param = 0;
      if (this.action == WindowAction.CLICK_ITEM) {
         param = (Integer)MagicValues.value(Integer.class, (Enum)this.param);
      } else if (this.action == WindowAction.SHIFT_CLICK_ITEM) {
         param = (Integer)MagicValues.value(Integer.class, (Enum)this.param);
      } else if (this.action == WindowAction.MOVE_TO_HOTBAR_SLOT) {
         param = (Integer)MagicValues.value(Integer.class, (Enum)this.param);
         ...
         param = (Integer)MagicValues.value(Integer.class, (Enum)this.param) + (this.slot != -999 ? 2 : 0);
      } else if (this.action == WindowAction.SPREAD_ITEM) {
         param = (Integer)MagicValues.value(Integer.class, (Enum)this.param);
      } else if (this.action == WindowAction.FILL_STACK) {
         param = (Integer)MagicValues.value(Integer.class, (Enum)this.param);
      ...
      out.writeByte((Integer)MagicValues.value(Integer.class, this.action));
