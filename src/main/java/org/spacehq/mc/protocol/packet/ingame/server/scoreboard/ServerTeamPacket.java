...
public class ServerTeamPacket implements Packet {
   ...
   public void read(NetInput in) throws IOException {
      ...
      this.action = (TeamAction)MagicValues.key(TeamAction.class, in.readByte());
      if (this.action == TeamAction.CREATE || this.action == TeamAction.UPDATE) {
         ...
         this.nameTagVisibility = (NameTagVisibility)MagicValues.key(NameTagVisibility.class, in.readString());
         this.color = (TeamColor)MagicValues.key(TeamColor.class, in.readByte());
      }
   ...
   public void write(NetOutput out) throws IOException {
      ...
      if (this.action == TeamAction.CREATE || this.action == TeamAction.UPDATE) {
         ...
         out.writeByte(flags);
         out.writeString((String)MagicValues.value(String.class, this.nameTagVisibility));
         out.writeByte((Integer)MagicValues.value(Integer.class, this.color));
      }

      if (this.action == TeamAction.CREATE || this.action == TeamAction.ADD_PLAYER || this.action == TeamAction.REMOVE_PLAYER) {
         out.writeVarInt(this.players.length);
         String[] var6 = this.players;
         int var3 = var6.length;

         for(int var4 = 0; var4 < var3; ++var4) {
            String player = var6[var4];
            if (player != null) {
