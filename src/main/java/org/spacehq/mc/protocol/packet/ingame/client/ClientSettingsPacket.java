...
public class ClientSettingsPacket implements Packet {
   ...
   public void read(NetInput in) throws IOException {
      ...
      this.chatVisibility = (ChatVisibility)MagicValues.key(ChatVisibility.class, in.readByte());
      this.chatColors = in.readBoolean();
      this.visibleParts = new ArrayList();
      int flags = in.readUnsignedByte();
      SkinPart[] var3 = SkinPart.values();
      int var4 = var3.length;

      for(int var5 = 0; var5 < var4; ++var5) {
         SkinPart part = var3[var5];
   ...
   public void write(NetOutput out) throws IOException {
      ...
      out.writeByte((Integer)MagicValues.value(Integer.class, this.chatVisibility));
      out.writeBoolean(this.chatColors);
      int flags = 0;

      SkinPart part;
      for(Iterator var3 = this.visibleParts.iterator(); var3.hasNext(); flags |= 1 << part.ordinal()) {
         part = (SkinPart)var3.next();
      }
