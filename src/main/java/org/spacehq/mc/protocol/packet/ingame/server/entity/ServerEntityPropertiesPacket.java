...
public class ServerEntityPropertiesPacket implements Packet {
   ...
   public void read(NetInput in) throws IOException {
      ...
      for(int index = 0; index < length; ++index) {
         ...
         for(int ind = 0; ind < len; ++ind) {
            modifiers.add(new AttributeModifier(in.readUUID(), in.readDouble(), (ModifierOperation)MagicValues.key(ModifierOperation.class, in.readByte())));
         }
   ...
   public void write(NetOutput out) throws IOException {
      ...
      while(var2.hasNext()) {
         ...
         Iterator var4 = attribute.getModifiers().iterator();

         while(var4.hasNext()) {
            AttributeModifier modifier = (AttributeModifier)var4.next();
            out.writeUUID(modifier.getUUID());
            out.writeDouble(modifier.getAmount());
            out.writeByte((Integer)MagicValues.value(Integer.class, modifier.getOperation()));
         }
      }
