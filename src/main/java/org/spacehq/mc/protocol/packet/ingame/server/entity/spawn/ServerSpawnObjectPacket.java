...
public class ServerSpawnObjectPacket implements Packet {
   ...
   public ServerSpawnObjectPacket(int entityId, ObjectType type, double x, double y, double z, float yaw, float pitch) {
      this(entityId, type, (ObjectData)null, x, y, z, yaw, pitch, 0.0D, 0.0D, 0.0D);
   }
   ...
   public void read(NetInput in) throws IOException {
      ...
      this.type = (ObjectType)MagicValues.key(ObjectType.class, in.readByte());
      this.x = (double)in.readInt() / 32.0D;
      this.y = (double)in.readInt() / 32.0D;
      this.z = (double)in.readInt() / 32.0D;
      this.pitch = (float)(in.readByte() * 360) / 256.0F;
      this.yaw = (float)(in.readByte() * 360) / 256.0F;
      int data = in.readInt();
      if (data > 0) {
         if (this.type == ObjectType.MINECART) {
            this.data = (ObjectData)MagicValues.key(MinecartType.class, data);
         } else if (this.type == ObjectType.ITEM_FRAME) {
            this.data = (ObjectData)MagicValues.key(HangingDirection.class, data);
         } else if (this.type == ObjectType.FALLING_BLOCK) {
            this.data = new FallingBlockData(data & '\uffff', data >> 16);
         ...
         this.motX = (double)in.readShort() / 8000.0D;
         this.motY = (double)in.readShort() / 8000.0D;
         this.motZ = (double)in.readShort() / 8000.0D;
      }
   ...
   public void write(NetOutput out) throws IOException {
      ...
      out.writeByte((Integer)MagicValues.value(Integer.class, this.type));
      ...
      if (this.data != null) {
         if (this.data instanceof MinecartType) {
            data = (Integer)MagicValues.value(Integer.class, (Enum)this.data);
