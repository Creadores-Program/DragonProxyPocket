      ...
      if (protocol.getSubProtocol() == SubProtocol.LOGIN) {
         if (event.getPacket() instanceof EncryptionRequestPacket) {
            ...
            try {
               ...
               event.getSession().disconnect("Login failed: Authentication service unavailable.", var10);
               return;
            } catch (InvalidCredentialsException var11) {
               event.getSession().disconnect("Login failed: Invalid login session.", var11);
               return;
            } catch (RequestException var12) {
               event.getSession().disconnect("Login failed: Authentication error: " + var12.getMessage(), var12);
               return;
            }
