...
public abstract class Metrics {
                     ...
                     if (this.nextPost == 0L || System.currentTimeMillis() > this.nextPost) {
                        try {
                           ...
                           if (Metrics.this.debug) {
                              System.out.println("[Metrics] " + var8.getMessage());
                           }
                        }
                     }
   ...
   public boolean isOptOut() {
      synchronized(this.optOutLock) {
         try {
            ...
            if (this.debug) {
               System.out.println("[Metrics] " + var4.getMessage());
            }
   ...
   private void postPlugin(boolean isPing) throws IOException {
      String serverVersion = this.getFullServerVersion();
      int playersOnline = this.getPlayersOnline();
      ...
      if (this.graphs.size() > 0) {
         synchronized(this.graphs) {
            ...
            for(Iterator iter = this.graphs.iterator(); iter.hasNext(); firstGraph = false) {
               ...
               graphJson.append('{');
               Iterator var15 = graph.getPlotters().iterator();

               while(var15.hasNext()) {
                  Metrics.Plotter plotter = (Metrics.Plotter)var15.next();
      ...
      json.append('}');
      URL url = new URL("http://report.mcstats.org" + String.format("/plugin/%s", urlEncode(this.pluginName)));
