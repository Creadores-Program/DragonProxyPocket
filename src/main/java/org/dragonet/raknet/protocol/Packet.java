...
public abstract class Packet implements Cloneable {
   ...
   protected InetSocketAddress getAddress() {
      ...
      if (version == 4) {
         String addr = (~this.getByte() & 255) + "." + (~this.getByte() & 255) + "." + (~this.getByte() & 255) + "." + (~this.getByte() & 255);
         int port = this.getSignedShort() & '\uffff';
   ...
   protected void putAddress(String addr, int port, byte version) {
      ...
      if (version == 4) {
         String[] var4 = addr.split("\\.");
         int var5 = var4.length;

         for(int var6 = 0; var6 < var5; ++var6) {
            String b = var4[var6];
            this.putByte((byte)(~Integer.valueOf(b) & 255));
         }
   ...
   public void encode() {
      this.buffer = new byte[]{this.getID()};
   }
