...
public enum EntityType {
   ...
   public static EntityType convertToPE(MobType pcType) {
      return convertToPE((Integer)MagicValues.value(Integer.class, pcType));
   }

   public static EntityType convertToPE(int pcType) {
      return !PC_TO_PE.containsKey(pcType) ? null : (EntityType)PC_TO_PE.get(pcType);
   }
   ...
   static {
      ...
      for(int var2 = 0; var2 < var1; ++var2) {
         EntityType peType = var0[var2];

         try {
            MobType pcType = MobType.valueOf(peType.name());
            int pcTypeId = (Integer)MagicValues.value(Integer.class, pcType);
