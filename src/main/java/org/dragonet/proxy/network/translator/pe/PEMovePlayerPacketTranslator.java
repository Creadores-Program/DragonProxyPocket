...
public class PEMovePlayerPacketTranslator implements PEPacketTranslator<MovePlayerPacket> {
   public Packet[] translate(UpstreamSession session, MovePlayerPacket packet) {
      ClientPlayerPositionRotationPacket pk = new ClientPlayerPositionRotationPacket(!packet.onGround, (double)packet.x, (double)packet.y, (double)packet.z, packet.yaw, packet.pitch);
      CachedEntity cliEntity = session.getEntityCache().getClientEntity();
      cliEntity.x = (double)packet.x;
      cliEntity.y = (double)packet.y;
      cliEntity.z = (double)packet.z;
