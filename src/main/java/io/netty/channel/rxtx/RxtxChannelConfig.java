...
public interface RxtxChannelConfig extends ChannelConfig {
   ...
   public static enum Paritybit {
      ...
      public static RxtxChannelConfig.Paritybit valueOf(int value) {
         RxtxChannelConfig.Paritybit[] arr$ = values();
         int len$ = arr$.length;

         for(int i$ = 0; i$ < len$; ++i$) {
            RxtxChannelConfig.Paritybit paritybit = arr$[i$];
   ...
   public static enum Databits {
      ...
      public static RxtxChannelConfig.Databits valueOf(int value) {
         RxtxChannelConfig.Databits[] arr$ = values();
         int len$ = arr$.length;

         for(int i$ = 0; i$ < len$; ++i$) {
            RxtxChannelConfig.Databits databit = arr$[i$];
   ...
   public static enum Stopbits {
      ...
      public static RxtxChannelConfig.Stopbits valueOf(int value) {
         RxtxChannelConfig.Stopbits[] arr$ = values();
         int len$ = arr$.length;

         for(int i$ = 0; i$ < len$; ++i$) {
            RxtxChannelConfig.Stopbits stopbit = arr$[i$];
