...
public class RxtxChannel extends OioByteStreamChannel {
   ...
   protected void doConnect(SocketAddress remoteAddress, SocketAddress localAddress) throws Exception {
      ...
      commPort.enableReceiveTimeout((Integer)this.config().getOption(RxtxChannelOption.READ_TIMEOUT));
      ...
      public void connect(SocketAddress remoteAddress, SocketAddress localAddress, final ChannelPromise promise) {
         if (promise.setUncancellable() && this.ensureOpen(promise)) {
            try {
               ...
               int waitTime = (Integer)RxtxChannel.this.config().getOption(RxtxChannelOption.WAIT_TIME);
               if (waitTime > 0) {
