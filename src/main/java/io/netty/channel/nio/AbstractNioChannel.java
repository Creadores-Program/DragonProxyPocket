...
public abstract class AbstractNioChannel extends AbstractChannel {
   ...
   protected void doRegister() throws Exception {
      ...
      while(true) {
         try {
            this.selectionKey = this.javaChannel().register(((NioEventLoop)this.eventLoop().unwrap()).selector, 0, this);
            return;
            ...
            ((NioEventLoop)this.eventLoop().unwrap()).selectNow();
            selected = true;
         }
      }
   }
   ...
   protected void doDeregister() throws Exception {
      ((NioEventLoop)this.eventLoop().unwrap()).cancel(this.selectionKey());
   }
