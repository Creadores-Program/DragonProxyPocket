...
public class OioSctpChannel extends AbstractOioMessageChannel implements SctpChannel {
   ...
   protected int doReadMessages(List<Object> msgs) throws Exception {
      if (!this.readSelector.isOpen()) {
         ...
         if (!keysSelected) {
            ...
            Set reableKeys = this.readSelector.selectedKeys();

            int var13;
            try {
               Iterator i$ = reableKeys.iterator();
               ...
               while(true) {
                  ...
                  SelectionKey ignored = (SelectionKey)i$.next();
                  allocHandle = this.unsafe().recvBufAllocHandle();
                  buffer = allocHandle.allocate(this.config().getAllocator());
                  free = true;
                  boolean var24 = false;
   ...
   public Set<InetSocketAddress> allLocalAddresses() {
      try {
         ...
         Iterator i$ = allLocalAddresses.iterator();
   ...
   private final class OioSctpChannelConfig extends DefaultSctpChannelConfig {
      ...
      // $FF: synthetic method
      OioSctpChannelConfig(OioSctpChannel x1, com.sun.nio.sctp.SctpChannel x2, Object x3) {
         this(x1, x2);
      }
   }
}
