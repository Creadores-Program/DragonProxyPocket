...
final class EpollEventLoop extends SingleThreadEventLoop {
   ...
   private int epollWait(boolean oldWakenUp) throws IOException {
      ...
      while(true) {
         ...
         ready = Native.epollWait(this.epollFd, this.events, (int)timeoutMillis);
         ++selectCnt;
         if (ready != 0 || oldWakenUp || this.wakenUp == 1 || this.hasTasks() || this.hasScheduledTasks()) {
            return ready;
         }
   ...
   private void closeAll() {
      ...
      Iterator i$ = this.channels.entries().iterator();

      while(i$.hasNext()) {
         IntObjectMap.Entry<AbstractEpollChannel> entry = (IntObjectMap.Entry)i$.next();
      ...
      while(i$.hasNext()) {
         AbstractEpollChannel ch = (AbstractEpollChannel)i$.next();
      ...
      for(int i = 0; i < ready; ++i) {
         ...
         if (fd == this.eventFd) {
            ...
            if (ch != null && ch.isOpen()) {
               boolean close = (ev & (long)Native.EPOLLRDHUP) != 0L;
               boolean read = (ev & (long)Native.EPOLLIN) != 0L;
               boolean write = (ev & (long)Native.EPOLLOUT) != 0L;
   ...
   protected void cleanup() {
      try {
         try {
            ...
            logger.warn("Failed to close the epoll fd.", (Throwable)var7);
         }

         try {
            Native.close(this.eventFd);
         } catch (IOException var6) {
            logger.warn("Failed to close the event fd.", (Throwable)var6);
         }
      } finally {
         this.events.free();
      }
