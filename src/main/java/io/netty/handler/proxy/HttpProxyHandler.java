...
public final class HttpProxyHandler extends ProxyHandler {
   ...
   protected void removeEncoder(ChannelHandlerContext ctx) throws Exception {
      ctx.pipeline().remove((ChannelHandler)this.codec.encoder());
   }

   protected void removeDecoder(ChannelHandlerContext ctx) throws Exception {
      ctx.pipeline().remove((ChannelHandler)this.codec.decoder());
   }
   ...
   protected Object newInitialMessage(ChannelHandlerContext ctx) throws Exception {
      ...
      SocketAddress proxyAddress = this.proxyAddress();
      if (proxyAddress instanceof InetSocketAddress) {
         InetSocketAddress hostAddr = (InetSocketAddress)proxyAddress;
         req.headers().set(HttpHeaderNames.HOST, (CharSequence)(hostAddr.getHostString() + ':' + hostAddr.getPort()));
      }

      if (this.authorization != null) {
         req.headers().set(HttpHeaderNames.PROXY_AUTHORIZATION, (CharSequence)this.authorization);
      }
