...
public final class Socks5ProxyHandler extends ProxyHandler {
      ...
      if (response instanceof Socks5InitialResponse) {
         ...
         if (res.authMethod() != Socks5AuthMethod.NO_AUTH && res.authMethod() != authMethod) {
            ...
            if (authMethod == Socks5AuthMethod.NO_AUTH) {
               ...
               ctx.pipeline().replace((String)this.decoderName, this.decoderName, new Socks5PasswordAuthResponseDecoder());
   ...
   private void sendConnectCommand(ChannelHandlerContext ctx) throws Exception {
      ...
      if (raddr.isUnresolved()) {
         ...
         if (NetUtil.isValidIpV4Address(rhost)) {
            ...
            if (!NetUtil.isValidIpV6Address(rhost)) {
               throw new ProxyConnectException(this.exceptionMessage("unknown address type: " + StringUtil.simpleClassName((Object)rhost)));
            }
      ...
      ctx.pipeline().replace((String)this.decoderName, this.decoderName, new Socks5CommandResponseDecoder());
