   ...
   private String sniHostNameFromHandshakeInfo(ByteBuf in) {
      ...
      try {
         ...
         case 22:
            ...
            if (majorVersion == 3) {
               ...
               if (in.readableBytes() >= packetLength) {
                  ...
                  int extensionLength;
                  for(int extensionsLimit = offset + extensionsLength; offset < extensionsLimit; offset += extensionLength) {
                     int extensionType = in.getUnsignedShort(offset);
                     offset += 2;
                     extensionLength = in.getUnsignedShort(offset);
                     offset += 2;
                     if (extensionType == 0) {
         ...
         if (logger.isDebugEnabled()) {
            logger.debug("Unexpected client hello packet: " + ByteBufUtil.hexDump(in), var16);
         }
