...
public class GlobalChannelTrafficShapingHandler extends AbstractTrafficShapingHandler {
   ...
   private void computeDeviationCumulativeBytes() {
      ...
      Iterator i$ = this.channelQueues.values().iterator();
      ...
      if (perChannel != null) {
         synchronized(perChannel) {
            ...
            if (channel.isActive()) {
               ...
               while(i$.hasNext()) {
                  toSend = (GlobalChannelTrafficShapingHandler.ToSend)i$.next();
   ...
   protected void submitWrite(final ChannelHandlerContext ctx, Object msg, long size, long writedelay, long now, ChannelPromise promise) {
      Channel channel = ctx.channel();
      Integer key = channel.hashCode();
      final GlobalChannelTrafficShapingHandler.PerChannel perChannel = (GlobalChannelTrafficShapingHandler.PerChannel)this.channelQueues.get(key);
      if (perChannel == null) {
   ...
   private void sendAllValid(ChannelHandlerContext ctx, GlobalChannelTrafficShapingHandler.PerChannel perChannel, long now) {
      synchronized(perChannel) {
         GlobalChannelTrafficShapingHandler.ToSend newToSend = (GlobalChannelTrafficShapingHandler.ToSend)perChannel.messagesQueue.pollFirst();
