...
final class Bzip2DivSufSort {
      ...
      while(true) {
         while(last - first > 8) {
            ...
            if (limit < 0) {
               ...
               a = this.ssPivot(depth, pa, first, last);
               v = T[depth + SA[pa + SA[a]]] & 255;
   ...
   private void ssMerge(int pa, int first, int middle, int last, int[] buf, int bufoffset, int bufsize, int depth) {
      ...
      while(true) {
         Bzip2DivSufSort.StackEntry entry;
      ...
      while(true) {
         ...
         if (limit < 0) {
            if (limit == -1) {
               ...
               Bzip2DivSufSort.StackEntry entry;
               if (first >= a && b >= last) {
                  for(c = first; c < last; SA[isa + SA[c]] = c++) {
                  }
            ...
            if (a > d) {
               ...
               if (a - first <= last - b) {
                  if (last - b <= b - a) {
                     if (1 < a - first) {
                        stack[ssize++] = new Bzip2DivSufSort.StackEntry(isaD + 1, a, b, next);
   ...
   private int sortTypeBstar(int[] bucketA, int[] bucketB) {
      ...
      int ti1;
      int t0;
      if ((ti = T[i] & 255) < (t0 = T[0] & 255) || T[i] == T[0] && flag) {
         if (!flag) {
      ...
      while(true) {
         do {
            if (0 > i) {
               ...
               for(i = m - 2; 0 <= i; SA[--bucketB[BUCKET_BSTAR(c0, c1)]] = i--) {
                  t = SA[PAb + i];
                  c0 = T[t] & 255;
                  c1 = T[t + 1] & 255;
               }
               ...
               while(true) {
                     ...
                     if (0 > i) {
                        ...
                        for(int k = m - 1; 0 <= c0; --c0) {
                           for(c1 = 255; c0 < c1; --c1) {
                              ...
                              for(j = bucketB[BUCKET_BSTAR(c0, c1)]; j <= k; --k) {
                                 SA[i] = SA[k];
                                 --i;
                              }
                           }
                  ...
                  for(SA[SA[ISAb + j]] = i--; 0 <= i && (T[i] & 255) <= (T[i + 1] & 255); --i) {
                  }
               }
            }
