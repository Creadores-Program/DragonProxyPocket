...
final class SpdySession {
   ...
   void acceptStream(int streamId, byte priority, boolean remoteSideClosed, boolean localSideClosed, int sendWindowSize, int receiveWindowSize, boolean remote) {
      if (!remoteSideClosed || !localSideClosed) {
         SpdySession.StreamState state = (SpdySession.StreamState)this.activeStreams.put(streamId, new SpdySession.StreamState(priority, remoteSideClosed, localSideClosed, sendWindowSize, receiveWindowSize));
         if (state == null) {
            if (remote) {
   ...
   private SpdySession.StreamState removeActiveStream(int streamId, boolean remote) {
      SpdySession.StreamState state = (SpdySession.StreamState)this.activeStreams.remove(streamId);
      if (state != null) {
         if (remote) {
   ...
   boolean isRemoteSideClosed(int streamId) {
      SpdySession.StreamState state = (SpdySession.StreamState)this.activeStreams.get(streamId);
   ...
   void updateAllSendWindowSizes(int deltaWindowSize) {
      Iterator i$ = this.activeStreams.values().iterator();

      while(i$.hasNext()) {
         SpdySession.StreamState state = (SpdySession.StreamState)i$.next();
   ...
   SpdySession.PendingWrite getPendingWrite(int streamId) {
      if (streamId == 0) {
         Iterator i$ = this.activeStreams().entrySet().iterator();

         while(i$.hasNext()) {
            Entry<Integer, SpdySession.StreamState> e = (Entry)i$.next();
