...
public class SpdySessionHandler extends ChannelHandlerAdapter {
   ...
   public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
      ...
      if (msg instanceof SpdyDataFrame) {
         ...
         deltaWindowSize = -1 * spdyDataFrame.content().readableBytes();
         newInitialWindowSize = this.spdySession.updateReceiveWindowSize(0, deltaWindowSize);
         ...
         int newWindowSize;
         if (newInitialWindowSize <= this.initialSessionReceiveWindowSize / 2) {
            newWindowSize = this.initialSessionReceiveWindowSize - newInitialWindowSize;
            this.spdySession.updateReceiveWindowSize(0, newWindowSize);
            SpdyWindowUpdateFrame spdyWindowUpdateFrame = new DefaultSpdyWindowUpdateFrame(0, newWindowSize);
         ...
         newWindowSize = this.spdySession.updateReceiveWindowSize(streamId, deltaWindowSize);
         ...
         streamId = spdySettingsFrame.getValue(0);
         if (streamId >= 0 && streamId != this.minorVersion) {
            this.issueSessionError(ctx, SpdySessionStatus.PROTOCOL_ERROR);
            return;
         }

         deltaWindowSize = spdySettingsFrame.getValue(4);
         if (deltaWindowSize >= 0) {
            this.remoteConcurrentStreams = deltaWindowSize;
         }
         ...
         newInitialWindowSize = spdySettingsFrame.getValue(7);
         if (newInitialWindowSize >= 0) {
   ...
   public void channelInactive(ChannelHandlerContext ctx) throws Exception {
      Iterator i$ = this.spdySession.activeStreams().keySet().iterator();

      while(i$.hasNext()) {
         Integer streamId = (Integer)i$.next();
   ...
   private void handleOutboundMessage(final ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
      ...
      if (msg instanceof SpdyDataFrame) {
         ...
         newConcurrentStreams = spdyDataFrame.content().readableBytes();
         newInitialWindowSize = this.spdySession.getSendWindowSize(streamId);
         int sessionSendWindowSize = this.spdySession.getSendWindowSize(0);
         newInitialWindowSize = Math.min(newInitialWindowSize, sessionSendWindowSize);
         if (newInitialWindowSize <= 0) {
            this.spdySession.putPendingWrite(streamId, new SpdySession.PendingWrite(spdyDataFrame, promise));
            return;
         }

         if (newInitialWindowSize < newConcurrentStreams) {
            this.spdySession.updateSendWindowSize(streamId, -1 * newInitialWindowSize);
            this.spdySession.updateSendWindowSize(0, -1 * newInitialWindowSize);
            SpdyDataFrame partialDataFrame = new DefaultSpdyDataFrame(streamId, spdyDataFrame.content().readSlice(newInitialWindowSize).retain());
            this.spdySession.putPendingWrite(streamId, new SpdySession.PendingWrite(spdyDataFrame, promise));
         ...
         this.spdySession.updateSendWindowSize(streamId, -1 * newConcurrentStreams);
         this.spdySession.updateSendWindowSize(0, -1 * newConcurrentStreams);
         ...
         newConcurrentStreams = spdySettingsFrame.getValue(4);
