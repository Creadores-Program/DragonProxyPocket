...
public class DefaultSpdyHeaders extends DefaultTextHeaders implements SpdyHeaders {
   private static final Headers.ValueConverter<CharSequence> SPDY_VALUE_CONVERTER = new DefaultTextHeaders.DefaultTextValueTypeConverter() {
      public CharSequence convertObject(Object value) {
         ...
         SpdyCodecUtil.validateHeaderValue((CharSequence)seq);
         return (CharSequence)seq;
      }
   };
   private static final DefaultHeaders.NameConverter<CharSequence> SPDY_NAME_CONVERTER = new DefaultHeaders.NameConverter<CharSequence>() {
      public CharSequence convertName(CharSequence name) {
         Object namex;
         if (name instanceof AsciiString) {
            namex = ((AsciiString)name).toLowerCase();
         } else {
            namex = name.toString().toLowerCase(Locale.US);
         }

         SpdyCodecUtil.validateHeaderName((CharSequence)namex);
         return (CharSequence)namex;
      }
   };
