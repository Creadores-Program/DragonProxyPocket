...
public class SpdyHttpDecoder extends MessageToMessageDecoder<SpdyFrame> {
   ...
   protected void decode(ChannelHandlerContext ctx, SpdyFrame msg, List<Object> out) throws Exception {
      int streamId;
      DefaultSpdyRstStreamFrame spdyRstStreamFrame;
      DefaultSpdyRstStreamFrame spdyRstStreamFrame;
      if (msg instanceof SpdySynStreamFrame) {
         ...
         if (SpdyCodecUtil.isServerId(streamId)) {
            ...
            if (associatedToStreamId == 0) {
               spdyRstStreamFrame = new DefaultSpdyRstStreamFrame(streamId, SpdyStreamStatus.INVALID_STREAM);
               ctx.writeAndFlush(spdyRstStreamFrame);
               return;
            }

            if (spdySynStreamFrame.isLast()) {
               spdyRstStreamFrame = new DefaultSpdyRstStreamFrame(streamId, SpdyStreamStatus.PROTOCOL_ERROR);
               ctx.writeAndFlush(spdyRstStreamFrame);
               return;
            }

            if (spdySynStreamFrame.isTruncated()) {
               spdyRstStreamFrame = new DefaultSpdyRstStreamFrame(streamId, SpdyStreamStatus.INTERNAL_ERROR);
               ctx.writeAndFlush(spdyRstStreamFrame);
               return;
            }
         ...
         FullHttpResponse fullHttpMessage;
         if (msg instanceof SpdySynReplyFrame) {
            ...
            try {
               fullHttpMessage = createHttpResponse(ctx, spdySynReplyFrame, this.validateHeaders);
            ...
            if (msg instanceof SpdyHeadersFrame) {
               ...
               fullHttpMessage = this.getMessage(streamId);
               ...
               if (spdyHeadersFrame.isLast()) {
                  HttpHeaderUtil.setContentLength(fullHttpMessage, (long)fullHttpMessage.content().readableBytes());
               ...
               if (spdyDataFrame.isLast()) {
                  HttpHeaderUtil.setContentLength(fullHttpMessage, (long)content.readableBytes());
                  this.removeMessage(streamId);
                  out.add(fullHttpMessage);
               }
            } else if (msg instanceof SpdyRstStreamFrame) {
               SpdyRstStreamFrame spdyRstStreamFrame = (SpdyRstStreamFrame)msg;
               streamId = spdyRstStreamFrame.streamId();
               this.removeMessage(streamId);
            }
         }
      }
   ...
   private static FullHttpRequest createHttpRequest(int spdyVersion, SpdyHeadersFrame requestFrame) throws Exception {
      ...
      req.headers().set(HttpHeaderNames.HOST, (CharSequence)host);
