...
public class DefaultHttp2FrameWriter implements Http2FrameWriter, Http2FrameSizePolicy, Http2FrameWriter.Configuration {
   ...
   public ChannelFuture writeData(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding, boolean endStream, ChannelPromise promise) {
      ...
      try {
         ...
         return promiseAggregator.setFailure(var12);
      }
   }
   ...
   public ChannelFuture writePriority(ChannelHandlerContext ctx, int streamId, int streamDependency, short weight, boolean exclusive, ChannelPromise promise) {
      try {
         ...
         long word1 = exclusive ? 2147483648L | (long)streamDependency : (long)streamDependency;
         Http2CodecUtil.writeUnsignedInt(word1, buf);
         buf.writeByte(weight - 1);
         return ctx.write(buf, promise);
      } catch (Throwable var10) {
         return promise.setFailure(var10);
      }
   }
   ...
   public ChannelFuture writePing(ChannelHandlerContext ctx, boolean ack, ByteBuf data, ChannelPromise promise) {
      ...
      try {
         ...
         return promiseAggregator.setFailure(var9);
      }
   }
   ...
   public ChannelFuture writePushPromise(ChannelHandlerContext ctx, int streamId, int promisedStreamId, Http2Headers headers, int padding, ChannelPromise promise) {
      ...
      try {
         ...
         int nonFragmentLength = 4 + padding + flags.getPaddingPresenceFieldLength();
         ...
         ChannelPromise var15 = promiseAggregator.doneAllocatingPromises();
         return var15;
      } catch (Throwable var19) {
         var10 = promiseAggregator.setFailure(var19);
      } finally {
         if (headerBlock != null) {
            headerBlock.release();
         }
   ...
   public ChannelFuture writeGoAway(ChannelHandlerContext ctx, int lastStreamId, long errorCode, ByteBuf debugData, ChannelPromise promise) {
      ...
      try {
         ...
         return promiseAggregator.setFailure(var11);
      }
   }
   ...
   public ChannelFuture writeFrame(ChannelHandlerContext ctx, byte frameType, int streamId, Http2Flags flags, ByteBuf payload, ChannelPromise promise) {
      ...
      try {
         ...
         return promiseAggregator.setFailure(var10);
      }
   }
   ...
   private ChannelFuture writeHeadersInternal(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int padding, boolean endStream, boolean hasPriority, int streamDependency, short weight, boolean exclusive, ChannelPromise promise) {
      ...
      try {
         ...
         ChannelPromise var27 = promiseAggregator.doneAllocatingPromises();
         return var27;
      } catch (Throwable var24) {
         var14 = promiseAggregator.setFailure(var24);
      } finally {
         if (headerBlock != null) {
            headerBlock.release();
         }
