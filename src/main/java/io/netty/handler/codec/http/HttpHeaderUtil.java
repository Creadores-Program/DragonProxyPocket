...
public final class HttpHeaderUtil {
   ...
   public static void setKeepAlive(HttpMessage message, boolean keepAlive) {
      ...
      if (message.protocolVersion().isKeepAliveDefault()) {
         if (keepAlive) {
            ...
            h.set(HttpHeaderNames.CONNECTION, (CharSequence)HttpHeaderValues.CLOSE);
         }
      } else if (keepAlive) {
         h.set(HttpHeaderNames.CONNECTION, (CharSequence)HttpHeaderValues.KEEP_ALIVE);
      } else {
         h.remove(HttpHeaderNames.CONNECTION);
      }
   ...
   public static boolean is100ContinueExpected(HttpMessage message) {
      if (!(message instanceof HttpRequest)) {
         ...
         if (value == null) {
            ...
            return HttpHeaderValues.CONTINUE.equalsIgnoreCase(value) ? true : message.headers().contains(HttpHeaderNames.EXPECT, HttpHeaderValues.CONTINUE, true);
         }
      }
   }
   ...
   public static void set100ContinueExpected(HttpMessage message, boolean expected) {
      if (expected) {
         message.headers().set(HttpHeaderNames.EXPECT, (CharSequence)HttpHeaderValues.CONTINUE);
      } else {
   ...
   public static void setTransferEncodingChunked(HttpMessage m, boolean chunked) {
      if (chunked) {
         m.headers().add(HttpHeaderNames.TRANSFER_ENCODING, (CharSequence)HttpHeaderValues.CHUNKED);
