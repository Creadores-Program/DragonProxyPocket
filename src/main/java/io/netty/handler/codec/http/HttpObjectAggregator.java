...
public class HttpObjectAggregator extends MessageAggregator<HttpObject, HttpMessage, HttpContent, FullHttpMessage> {
   ...
   protected Object newContinueResponse(HttpMessage start) throws Exception {
      return HttpHeaderUtil.is100ContinueExpected(start) ? CONTINUE : null;
   }
   ...
   protected FullHttpMessage beginAggregation(HttpMessage start, ByteBuf content) throws Exception {
      ...
      if (start instanceof HttpRequest) {
         ret = new HttpObjectAggregator.AggregatedFullHttpRequest((HttpRequest)start, content, (HttpHeaders)null);
      } else {
         if (!(start instanceof HttpResponse)) {
            throw new Error();
         }

         ret = new HttpObjectAggregator.AggregatedFullHttpResponse((HttpResponse)start, content, (HttpHeaders)null);
      }
   ...
   protected void finishAggregation(FullHttpMessage aggregated) throws Exception {
      if (!HttpHeaderUtil.isContentLengthSet(aggregated)) {
         aggregated.headers().set(HttpHeaderNames.CONTENT_LENGTH, (CharSequence)String.valueOf(aggregated.content().readableBytes()));
      }
   ...
   private static final class AggregatedFullHttpResponse extends HttpObjectAggregator.AggregatedFullHttpMessage implements FullHttpResponse {
      ...
      private FullHttpResponse copy(boolean copyContent, ByteBuf newContent) {
         DefaultFullHttpResponse copy = new DefaultFullHttpResponse(this.protocolVersion(), this.status(), copyContent ? this.content().copy() : (newContent == null ? Unpooled.buffer(0) : newContent));
      ...
      public FullHttpResponse duplicate() {
         DefaultFullHttpResponse duplicate = new DefaultFullHttpResponse(this.protocolVersion(), this.status(), this.content().duplicate());
