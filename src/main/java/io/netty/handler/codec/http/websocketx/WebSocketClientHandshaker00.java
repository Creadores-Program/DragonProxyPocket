...
public class WebSocketClientHandshaker00 extends WebSocketClientHandshaker {
   ...
   protected FullHttpRequest newHandshakeRequest() {
      ...
      headers.add(HttpHeaderNames.UPGRADE, (CharSequence)WEBSOCKET).add(HttpHeaderNames.CONNECTION, (CharSequence)HttpHeaderValues.UPGRADE).add(HttpHeaderNames.HOST, (CharSequence)wsURL.getHost());
      ...
      headers.add(HttpHeaderNames.ORIGIN, (CharSequence)originValue).add(HttpHeaderNames.SEC_WEBSOCKET_KEY1, (CharSequence)key1).add(HttpHeaderNames.SEC_WEBSOCKET_KEY2, (CharSequence)key2);
      String expectedSubprotocol = this.expectedSubprotocol();
      if (expectedSubprotocol != null && !expectedSubprotocol.isEmpty()) {
         headers.add(HttpHeaderNames.SEC_WEBSOCKET_PROTOCOL, (CharSequence)expectedSubprotocol);
      }
   ...
   private static String insertRandomCharacters(String key) {
      ...
      while(true) {
         ...
         do {
            if (randCount >= count) {
               for(rand = 0; rand < count; ++rand) {
                  int split = WebSocketUtil.randomNumber(0, key.length());
                  String part1 = key.substring(0, split);
                  String part2 = key.substring(split);
                  key = part1 + randomChars[rand] + part2;
               }

               return key;
            }

            rand = (int)(Math.random() * 126.0D + 33.0D);
         } while((33 >= rand || rand >= 47) && (58 >= rand || rand >= 126));
