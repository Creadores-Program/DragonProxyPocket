...
public class DefaultHttpHeaders extends DefaultTextHeaders implements HttpHeaders {
   ...
   private static final DefaultHttpHeaders.HttpHeadersValidationConverter VALIDATE_OBJECT_CONVERTER;
   private static final DefaultHttpHeaders.HttpHeadersValidationConverter NO_VALIDATE_OBJECT_CONVERTER;
   private static final DefaultHttpHeaders.HttpHeadersNameConverter VALIDATE_NAME_CONVERTER;
   private static final DefaultHttpHeaders.HttpHeadersNameConverter NO_VALIDATE_NAME_CONVERTER;
   ...
   static {
      ...
      LOOKUP_TABLE[59] = -1;
      LOOKUP_TABLE[61] = -1;
      VALIDATE_OBJECT_CONVERTER = new DefaultHttpHeaders.HttpHeadersValidationConverter(true);
      NO_VALIDATE_OBJECT_CONVERTER = new DefaultHttpHeaders.HttpHeadersValidationConverter(false);
      VALIDATE_NAME_CONVERTER = new DefaultHttpHeaders.HttpHeadersNameConverter(true);
      NO_VALIDATE_NAME_CONVERTER = new DefaultHttpHeaders.HttpHeadersNameConverter(false);
   }
   ...
   static class HttpHeadersNameConverter implements DefaultHeaders.NameConverter<CharSequence> {
      ...
      private static void validateNameChar(CharSequence name, int character) {
         if ((character & -64) == 0 && DefaultHttpHeaders.LOOKUP_TABLE[character] != 0) {
   ...
   private static final class HttpHeadersValidationConverter extends DefaultTextHeaders.DefaultTextValueTypeConverter {
         ...
         if (value == null) {
            ...
            if (this.validate) {
               if (value instanceof AsciiString) {
                  ...
                  validateValue((CharSequence)seq);
               }
            }

            return (CharSequence)seq;
         }
      }
      ...
      private static void validateValue(AsciiString seq) {
         ...
         for(int index = start; index < end; ++index) {
            state = validateValueChar(seq, state, (char)(array[index] & 255));
         }
