...
public abstract class HttpContentEncoder extends MessageToMessageCodec<HttpRequest, HttpObject> {
         ...
         if (isPassthru(res)) {
            ...
            if (isFull && !((ByteBufHolder)res).content().isReadable()) {
               ...
               if (result == null) {
                  ...
                  res.headers().set(HttpHeaderNames.CONTENT_ENCODING, (CharSequence)result.targetContentEncoding());
                  res.headers().remove(HttpHeaderNames.CONTENT_LENGTH);
                  res.headers().set(HttpHeaderNames.TRANSFER_ENCODING, (CharSequence)HttpHeaderValues.CHUNKED);
                  if (isFull) {
   ...
   protected abstract HttpContentEncoder.Result beginEncode(HttpResponse var1, CharSequence var2) throws Exception;
