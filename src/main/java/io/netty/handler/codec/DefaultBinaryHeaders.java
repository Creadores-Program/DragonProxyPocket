...
public class DefaultBinaryHeaders extends DefaultHeaders<AsciiString> implements BinaryHeaders {
   private static final DefaultHeaders.HashCodeGenerator<AsciiString> ASCII_HASH_CODE_GENERATOR = new DefaultHeaders.HashCodeGenerator<AsciiString>() {
      public int generateHashCode(AsciiString name) {
         return AsciiString.caseInsensitiveHashCode(name);
      }
   };
   private static final Headers.ValueConverter<AsciiString> OBJECT_TO_ASCII = new Headers.ValueConverter<AsciiString>() {
      public AsciiString convertObject(Object value) {
         if (value instanceof AsciiString) {
            return (AsciiString)value;
         } else {
            return value instanceof CharSequence ? new AsciiString((CharSequence)value) : new AsciiString(value.toString());
         }
      }
   ...
   private static final DefaultHeaders.NameConverter<AsciiString> ASCII_TO_LOWER_CONVERTER = new DefaultHeaders.NameConverter<AsciiString>() {
      public AsciiString convertName(AsciiString name) {
         return name.toLowerCase();
      }
   };
   private static final DefaultHeaders.NameConverter<AsciiString> ASCII_IDENTITY_CONVERTER = new DefaultHeaders.NameConverter<AsciiString>() {
      public AsciiString convertName(AsciiString name) {
         return name;
      }
   };
   ...
   public BinaryHeaders addObject(AsciiString name, Iterable<?> values) {
      super.addObject(name, (Iterable)values);
      return this;
   }

   public BinaryHeaders addObject(AsciiString name, Object... values) {
      super.addObject(name, (Object[])values);
      return this;
   }
