...
public final class DnsNameResolverGroup extends NameResolverGroup<InetSocketAddress> {
   ...
   public DnsNameResolverGroup(Class<? extends DatagramChannel> channelType, InetSocketAddress localAddress, InetSocketAddress nameServerAddress) {
      this((ChannelFactory)(new ReflectiveChannelFactory(channelType)), localAddress, (InetSocketAddress)nameServerAddress);
   }
   ...
   public DnsNameResolverGroup(Class<? extends DatagramChannel> channelType, InetSocketAddress localAddress, Iterable<InetSocketAddress> nameServerAddresses) {
      this((ChannelFactory)(new ReflectiveChannelFactory(channelType)), localAddress, (Iterable)nameServerAddresses);
   }
   ...
   protected NameResolver<InetSocketAddress> newResolver(EventExecutor executor) throws Exception {
      if (!(executor instanceof EventLoop)) {
         throw new IllegalStateException("unsupported executor type: " + StringUtil.simpleClassName((Object)executor) + " (expected: " + StringUtil.simpleClassName(EventLoop.class));
      } else {
