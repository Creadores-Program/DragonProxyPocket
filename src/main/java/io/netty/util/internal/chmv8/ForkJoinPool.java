...
public class ForkJoinPool extends AbstractExecutorService {
   ...
   private void tryAddWorker() {
      while(true) {
         ...
         if ((u = (int)((c = this.ctl) >>> 32)) < 0 && (u & '耀') != 0 && (e = (int)c) >= 0) {
            ...
            try {
               ForkJoinPool.ForkJoinWorkerThreadFactory fac;
   ...
   final ForkJoinPool.WorkQueue registerWorker(ForkJoinWorkerThread wt) {
      wt.setDaemon(true);
      ...
      label120: {
         w = new ForkJoinPool.WorkQueue(this, wt, this.mode, s);
         ...
         if ((ws = this.workQueues) != null) {
            ...
            if (ws[r &= m] != null) {
               ...
               while(ws[r = r + step & m] != null) {
                  ++probes;
                  if (probes >= n) {
                     this.workQueues = ws = (ForkJoinPool.WorkQueue[])Arrays.copyOf(ws, n <<= 1);
                     m = n - 1;
                     probes = 0;
                  }
               }
            ...
            while(true) {
               ...
               if (ps != 0 && (ws = this.workQueues) != null && (m = ws.length - 1) >= 0) {
                  ...
                  label214: {
                     ...
                     s |= s >>> 16;
                     s = s + 1 << 1;
                     nws = (ws = this.workQueues) != null && ws.length != 0 ? null : new ForkJoinPool.WorkQueue[s];
                     if (((ps = this.plock) & 2) == 0) {
                        var10003 = ps;
                        ps += 2;
                        if (U.compareAndSwapInt(this, PLOCK, var10003, ps)) {
   ...
   private int tryHelpStealer(ForkJoinPool.WorkQueue joiner, ForkJoinTask<?> task) {
      ...
      if (task != null && joiner != null && joiner.base - joiner.top >= 0) {
         ...
         while(true) {
            ...
            ForkJoinPool.WorkQueue v;
            int s;
            for(ForkJoinPool.WorkQueue j = joiner; (s = task.status) >= 0; j = v) {
               ForkJoinPool.WorkQueue[] ws;
               int m;
   ...
   static int getSurplusQueuedTaskCount() {
      ...
      if ((t = Thread.currentThread()) instanceof ForkJoinWorkerThread) {
         ...
         int p;
         return n - (a > (p = p >>> 1) ? 0 : (a > (p >>>= 1) ? 1 : (a > (p >>>= 1) ? 2 : (a > (p >>>= 1) ? 4 : 8))));
      } else {
         return 0;
      }
   }
   ...
   static ForkJoinPool.WorkQueue commonSubmitterQueue() {
      ...
      ForkJoinPool.WorkQueue[] ws;
      int m;
      return (z = (ForkJoinPool.Submitter)submitters.get()) != null && (p = common) != null && (ws = p.workQueues) != null && (m = ws.length - 1) >= 0 ? ws[m & z.seed & 126] : null;
   }
