...
public final class InternalThreadLocalMap extends UnpaddedInternalThreadLocalMap {
   ...
   public static InternalThreadLocalMap get() {
      Thread thread = Thread.currentThread();
      return thread instanceof FastThreadLocalAccess ? fastGet((FastThreadLocalAccess)thread) : slowGet();
   }
   ...
   public static void remove() {
      Thread thread = Thread.currentThread();
      if (thread instanceof FastThreadLocalAccess) {
         ((FastThreadLocalAccess)thread).setThreadLocalMap((InternalThreadLocalMap)null);
      } else {
         ThreadLocal<InternalThreadLocalMap> slowThreadLocalMap = UnpaddedInternalThreadLocalMap.slowThreadLocalMap;
         if (slowThreadLocalMap != null) {
