...
public final class DefaultExecutorServiceFactory implements ExecutorServiceFactory {
   ...
   public ExecutorService newExecutorService(int parallelism) {
      ForkJoinPool.ForkJoinWorkerThreadFactory threadFactory = new DefaultExecutorServiceFactory.DefaultForkJoinWorkerThreadFactory(this.namePrefix + '-' + executorId.getAndIncrement());
      return new ForkJoinPool(parallelism, threadFactory, DefaultExecutorServiceFactory.DefaultUncaughtExceptionHandler.INSTANCE, true);
   }
   ...
   private static String toName(Class<?> clazz) {
      if (clazz == null) {
         ...
         default:
            return Character.isUpperCase(clazzName.charAt(0)) && Character.isLowerCase(clazzName.charAt(1)) ? Character.toLowerCase(clazzName.charAt(0)) + clazzName.substring(1) : clazzName;
         }
      }
   }
   ...
   private static final class DefaultUncaughtExceptionHandler implements UncaughtExceptionHandler {
      private static final DefaultExecutorServiceFactory.DefaultUncaughtExceptionHandler INSTANCE = new DefaultExecutorServiceFactory.DefaultUncaughtExceptionHandler();
